...
def EstimacionRacimoCicloAnterior(fec, estacion, rPA):
    pais = 3 #1 peru
    ...
    for k in datos_new:
        ...
        Vector_datos.append((fecha, round(GDA_acum,1) ,round(ES_ACUMULADO,1), round(RAD_SOLAR,2)))
        if GDA_acum >= 900:
            break
    semanas=round((len(Vector_datos))/7, 1)
    ...
    return fec_string, biomasa_planta, biomasa, semanas, Vector_datos
